...
public class FindVariableDefinitionVisitor implements Visitor {
	...
	public FindVariableDefinitionVisitor() {
	}
	...
	public void run(Id node) {
		if ((node.getParent() != null)
				&& (node.getParent() instanceof tree.AtomicNamedDeclarator)
				&& (node.getParent().getParent() != null)
				&& (node.getParent().getParent() instanceof tree.InitDeclaratorI)
				&& (node.getParent().getParent().getParent() != null)
				&& (node.getParent().getParent().getParent() instanceof tree.Declaration)
				&& (node.getParent().getParent().getParent().getParent() != null)
				&& (node.getParent().getParent().getParent().getParent() instanceof tree.DeclarationStatement)) {
			// This is a common local variable declaration
			// Excludes C99 parameter declaration
			this.localVariables.add(node);
		} else if ((node.getParent() != null)
				&& (node.getParent() instanceof tree.PostfixExpr)
				&& (node.getParent().getChildren().size() > 1)
				&& (node.getParent().getChildren().get(1) instanceof tree.SimplePostfixSuffix)
				) {
			// Variable increment/decrement: a++, a--
			// Excludes function calls
			this.localVariables.add(node);
		} else if ((node.getParent() != null)
				&& (node.getParent() instanceof tree.UnaryExpr)
				&& ((((tree.UnaryExpr) node.getParent()).getKind().equals("++"))
					|| ((((tree.UnaryExpr) node.getParent()).getKind().equals("--"))))) {
			// Variable increment/decrement: ++a, --a
			this.localVariables.add(node);
		}
