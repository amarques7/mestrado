...
public class FindFunctionDirectivesVisitor implements Visitor {
	...
	private Set<Opt> functionOpts = new HashSet<Opt>();
	
	public FindFunctionDirectivesVisitor(FunctionDef functionDef) {
	...
	public Set<Opt> getFunctionOpts() {
		return functionOpts;
	}
	...
	public void run(Opt node) {
		if (!(node.getConditional().isTautology())) {
			...
			if (!(((Opt) node).getConditional().equivalentTo(this.functionDef.getPresenceCondition()))) {
				this.functionOpts.add(node);
			}
		}
