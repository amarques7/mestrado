...
public class FindLocalVariableUsesVisitor implements Visitor {

	private Id variable;
	private List<Id> localVariableUses = new ArrayList<Id>();
	
	public Id getVariable() {
		return variable;
	}
	
	public List<Id> getLocalVariableUses() {
		return localVariableUses;
	}

	public FindLocalVariableUsesVisitor(Id variable) {
	...
	public void run(Id node) {
		...
		if ((!(node.getPositionFrom().$less(getVariable().getPositionFrom()))) && // use must be after definition
			(!((node.getParent().getChildren().size() > 1) && (node.getParent().getChildren().get(1) instanceof FunctionCall))) && // not a function call
			...
			(getVariable().getName().equals(node.getName()))) { // same name as the requested variable			
					this.localVariableUses.add(node);
		}
		for (int i = 0; i < node.getChildren().size(); i++){
			node.getChildren().get(i).accept(this);
		}
