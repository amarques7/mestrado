...
public class FindVariablesInFunctionCallArgumentsVisitor implements Visitor {

	private List<Id> functionCallVariables = new ArrayList<Id>();

	public List<Id> getFunctionCallVariables() {
		return functionCallVariables;
	}
	...
	public void run(Id node) {
		...
		if ((!((node.getParent().getChildren().size() > 1) && (node.getParent().getChildren().get(1) instanceof FunctionCall))) && // not a function call
			...
			(!(node.getParent() instanceof PointerPostfixSuffix))) {
			this.functionCallVariables.add(node);
		}
