...
public class FindFunctionCallsVisitor implements Visitor {
	...
	public FindFunctionCallsVisitor(Map<String, Function> orphanFunctionsMap) {
		this.orphanFunctionsMap = orphanFunctionsMap;
	}
	...
	public void run(FunctionCall node) {
		// If looking for one function only
		if (orphanFunctionsMap.isEmpty()) {
		...
		} else { // If looking for a set of functions
			if ((node.getParent().getChildren().get(0) instanceof Id)
					&& (orphanFunctionsMap.keySet().contains(((Id) node.getParent().getChildren().get(0)).getName()))) {
				orphanFunctionsMap.get(((Id) node.getParent().getChildren().get(0)).getName()).getFunctionCalls().add(node);
			}
		}
